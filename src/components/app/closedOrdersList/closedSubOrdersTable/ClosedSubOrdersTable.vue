...
import {
  ...
  ClosedSubOrdersTableProps,
  ClosedSubOrderTableItem,
} from './index';

const props = defineProps<ClosedSubOrdersTableProps>();

const { t } = useI18n();

const columns = ref<ClosedSubOrdersColumn[]>([
  {
    label: '',
    slug: 'type',
    size: 1,
...
const records = computed<ClosedSubOrdersRecord[]>(
  () => props.orders.map((order: ClosedSubOrderTableItem) => {
    const label = ({
